...
module.exports = createCoreController('api::disability-type.disability-type', ({ strapi }) => ({
    ...
    async find(ctx) {
      ...
      const newData = data.map((entry) => ({
        ...
        //attributes: {
          ...
          ...entry.attributes,
          students: entry.attributes.students && entry.attributes.students.data ? entry.attributes.students.data.map(embedIdInAttributes) : [],
        //},
      }));
  ...
  async findOne(ctx) {
    ...
    const newData = {
      //attributes: {
        ...
        ...data.attributes,
        students: data.attributes.students && data.attributes.students.data ? data.attributes.students.data.map(embedIdInAttributes) : [],
      // },
    };
    return newData;
  },
